...
from data_down_events_training import make_estimate_data
...
def build_model(events, domain, num_observations, M=20):
    ...
    kernel = gpflow.kernels.SquaredExponential(variance = 1.0, lengthscales = 0.1)  #0.01,0.05,0.1,0.2
...
def demo():
    ...
    model = build_model(events, domain, num_observations, M=10)   #M是 inducing point的数量
